...
Split two self-contained subsystems out of the ~2,800-line `initSgHub()` closure in `hub.js` into their own modules: the plain-English civic **glossary** (`static/js/glossary.js`, with its own tooltip) and the generic dependency-free **SVG chart engine** (`static/js/charts.js` — `renderLineChart` / `renderBarChart` / `renderScatterChart`, tooltip and tick helpers, and the dataviz colour constants). `hub.js` retains only the dashboard controllers that call them, dropping from ~2,806 to ~2,457 lines (~12%). While extracting the glossary, **fixed a latent `ReferenceError`**: it referenced an undeclared `hubPaneEl`, which silently disabled the entire feature (0 terms annotated); it now resolves `#hub-pane` and annotates 70+ acronyms live. All eight hub charts (COE, HDB resale, job-vacancy, retrenchment, job-history bar, occupational-wage scatter) were verified rendering after the split.
...
The **Gov Updates** tab streams from **21 unique official Telegram channels** (Elections Dept, CPF, LTA, SkillsFuture, SWDA, NEA, NLB, URA, GovTech, ScamShield, HealthHub, MOH, SPF, SCDF, MOM, Reach, etc.), and includes a dedicated **CDC / RedeemSG Media Releases** card — since CDC has no Telegram channel, it's a BeautifulSoup scrape of the `cdc.gov.sg` newsroom (a trusted-domain allowlist entry). The **IRAS Tax & Wealth** tab includes an **IRAS News & Updates** card from the official `@irassg` channel (kept at the bottom of the tab, below the deadlines and the relief optimizer). The **Kiasu SG Deals** community feed streams from **18 channels** (including `@confirmgood`, `@goodlobang`, `@kiasufoodies`, `@goodyfeedsg`, `@TSLMedia`, `@todayonlinesg`). All render with the same live/last-known freshness badge as the other scraper-backed cards. The Gov Updates feed streams broadcasts published strictly within the **last 3 days (72 hours)** across all 21 channels (with quiet channels returning 0 posts so old posts do not contaminate the 72h feed, and a single-latest fallback triggering only when zero posts are returned across all channels in 3 days), fronted by a **window hint chip** (`🕐 Official broadcasts from the last 3 days (72 hours) · N posts`).
...
**🔗 Anti-phishing link policy for Co-Pilot sources**
Tightened how the Co-Pilot renders sources so it never trains users to click links from a chat window. Only official Singapore government domains are now clickable — `*.gov.sg` plus the same short trusted allowlist the backend scraper uses (`healthhub.sg`, `wsg.sg`, `cdc.gov.sg`) — in both the citation pills *and* inline `[label](url)` references in the answer text (`static/js/chat.js`, `isTrustedGovURL`). Every other source (arbitrary Google Search Grounding results, blogs, forums) renders as plain, non-clickable text with its domain still visible, so the reader can see where a claim came from and open it themselves. Auth/login pages (SingPass/CorpPass/`login`/`auth` URLs) stay **never-clickable** even on a trusted host, carrying a shield marker — extending the earlier login-URL de-linking into a full allowlist that mirrors the backend `is_trusted_sg_domain` allowlist and `AUTH_URL_KEYWORDS` denylist. Also added a **BTO vs Resale** Co-Pilot prompt chip to the New-citizen persona.

**🧪 Testing**
Suite grown to **145 Python + 6 JavaScript tests**, including the new CDC/IRAS feed endpoints and the BigQuery-first resale/OWS paths (with the fallback tier exercised deterministically regardless of whether the machine running CI has live BigQuery credentials).
