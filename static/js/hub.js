// hub.js — SG Hub live dashboards (transit, gov/community feeds, HDB, jobs, wages,
// weather) plus the glossary tooltips, and the DOMContentLoaded that boots the hub.
...
function initSgHub() {
    ...
    // Renders a small freshness pill from a backend data_status marker ({is_live, synced_at, note}).
    // Live sources get a subtle green "Live" dot; when a scraper falls back to cached/sample data
    // the panel shows an amber "Showing last known data" pill so a stale source is never presented
    // as if it were current — matters most during a live demo when an upstream site is flaky.
    ...
    // SG Hub chart engine moved to its own module: static/js/charts.js
