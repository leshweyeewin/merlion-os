...
* **Robust CI/CD:** Guarded by a **293-test suite (287 Python, 6 JS)** — including a live golden-set retrieval-quality gate that auto-skips without an API key — plus pyflakes linting, a daily GitHub Action that monitors every live scraper, BigQuery dataset, **and hand-maintained policy figure (e.g. the benefit amounts, which drift at each Budget)** for silent breakage or staleness — opening/refreshing a single assigned, emailed GitHub issue on any failure — and an hourly Action that refreshes fallback data seeds.
