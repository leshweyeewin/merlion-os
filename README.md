...
* **RAG Civic Knowledge Base:** Uses `gemini-embedding-001` and pure-Python cosine similarity over a **109-chunk** curated civic corpus (spanning CPF, IRAS, HDB, MOM, MOH, MOE, LTA, ICA, NEA and benefits schemes) to accurately ground open-ended policy questions. Retrieval quality is measured by a golden-set test.
...
* **🌐 National 4-Language Localization & Elderly Mode:** Full UI and statutory card localization for Singapore's 4 official languages—**English**, **Chinese (中文)**, **Malay (Bahasa Melayu)**, and **Tamil (தமிழ்)**—standardized against official Singapore Government Glossaries (HDB *预购组屋*, IRAS *缴税通知单 NOA*, CPF *结单*, CDC *社理会消费券*). Includes a 1-click **Elderly Accessibility Mode** scaling typography to 16px+ with high-contrast borders for senior citizens.
...
**🛠️ Modular Tools Architecture (`tools/`)**
* **15 Specialized Tool Modules:** Backend logic is modularized into `tools/` — `civic.py` (grants/benefits finder), `search.py` (domain-validated web scraper), `environment.py` (NEA weather/PSI), `housing.py` (HDB BTO & resale forecasting), `transport.py` (LTA transit & COE forecasting), `jobs.py` (MOM BigQuery analytics), `wages.py` (MOM wage benchmarks), `knowledge.py` (RAG vector search), `chat.py` (Gemini streaming & failover ladder), `security.py` (PII Guardrails), `iras.py` (Tax & SRS optimizer), `cpf.py` (CPF LIFE estimator), `scam_checker.py` (5-layer ScamShield phishing detector), `alerts.py` (watchlist threshold evaluator), and `core.py` (shared CSV fetch/caching helpers). See [`docs/architecture_diagram.md`](docs/architecture_diagram.md) for full registry details.
...
|---|---|---|
| 🏛️ **Statutory Portals Directory** | All **93** agency portals list, drag-and-drop ordering, and portal search/multi-select panels. | [docs/portals.md](docs/portals.md) |
| 🎯 **Decision Engine & Life Events** | Unified "Money Left on the Table" Benefits Finder, end-to-end life milestone orchestration, and forward-looking decision foresight (HDB affordability, CPF LIFE, COE timing). | [docs/decision_engine.md](docs/decision_engine.md) |
| 🔔 **Watchlists & ScamShield** | Proactive trigger alerts (COE drop, BTO launch, resale shift, tax deadline, MRT delay) via Telegram/Web Push, and Community ScamShield phishing checker. | [docs/watchlists_and_scamshield.md](docs/watchlists_and_scamshield.md) |
| 🤖 **Co-Pilot & Document Copilot** | Gemini multimodal vision, PDF PII redaction pipeline, synthetic mock statement generators, persona profiles, and a Telegram-hosted Co-Pilot (ask questions + scam checks + alerts). | [docs/copilot_guide.md](docs/copilot_guide.md) |
| 🌐 **Localization & Accessibility** | 4-language (EN, ZH, MS, TA) Singov glossary standardization, plain-English intent search engine, life-stage personas, and Elderly Mode. | [docs/localization_and_accessibility.md](docs/localization_and_accessibility.md) |
...
### 3. Run Tests
Ensure dependencies are installed, then run the lint gate and the python/javascript test suites (383 Python + 6 JavaScript tests):
```bash
