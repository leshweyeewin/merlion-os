...
async def handle_whatsapp_message(client_id: str, phone: str, text: str) -> str:
    """Route one inbound WhatsApp message (pairing / commands / scam / chat) and return the bot's
    reply text. Shared by the in-app simulator (/api/whatsapp/message) and the real Cloud API
...
async def whatsapp_post_message(request: WhatsAppMessageRequest):
    ...
    try:
        reply = await handle_whatsapp_message(client_id, phone, text)
        return {"reply": reply}
    except Exception as e:
...
async def whatsapp_webhook_verify(request: Request):
    ...
    if (params.get("hub.mode") == "subscribe"
        ...
        # Echo the challenge as raw text
        return PlainTextResponse(params.get("hub.challenge", ""))
    raise HTTPException(status_code=403, detail="verification failed")


def _verify_wa_signature(raw: bytes, header: str) -> bool:
    import hmac
    import hashlib
    from tools.alert_delivery import whatsapp_enabled
    secret = os.environ.get("WHATSAPP_APP_SECRET")
    if not secret:
        # No secret set: allow in dev/simulator mode so local testing works, but once WhatsApp is
        # live (token + phone id configured) never accept an unsigned webhook — fail closed, so a
        # misconfigured prod can't be driven by forged inbound payloads.
        return not whatsapp_enabled()
...
async def whatsapp_webhook_receive(request: Request):
    ...
    if not _verify_wa_signature(raw, request.headers.get("X-Hub-Signature-256", "")):
    ...
    from tools import alerts as _alerts
    from tools.alert_delivery import send_whatsapp_message
    
    for entry in data.get("entry", []):
        for change in entry.get("changes", []):
            value = change.get("value", {})
            for m in value.get("messages", []):          # absent for status callbacks — skip those
                if m.get("type") != "text":
                    continue
                phone = m["from"]                         # already digits, e.g. 6591234567
                text = m["text"]["body"].strip()
                cid = _alerts.client_id_for_whatsapp(phone) or phone
                reply = await handle_whatsapp_message(cid, phone, text)
                if reply:
                    # send_whatsapp_message is sync (requests.post); off-load it so a slow Graph API
                    # call can't block the event loop. Inbound reply is inside the 24h window → plain text.
                    await anyio.to_thread.run_sync(send_whatsapp_message, phone, reply)
    return {"status": "ok"}
